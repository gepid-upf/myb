 ...
 * 
 * @copyright 2017 Raivis Strogonovs (https://morf.lv)
 * 
...
#include "max30100/max30100_registers.h"

const char *TAG_MAX30100 = "MAX30100";

esp_err_t max30100_init
(
    max30100_config_t* this,
    i2c_port_t i2c_num,
...
)
{
    ...
    this->red_current_adj_ms = MAX30100_DEFAULT_RED_LED_CURRENT_ADJUST_MS;
...
esp_err_t max30100_update(max30100_config_t* this, max30100_data_t* data)
{
    ...
    this->dc_filter_ir = max30100_dc_removal
    (
        (float) raw_data.raw_ir,
        this->dc_filter_ir.w,
        this->dc_alpha
    );
    this->dc_filter_red = max30100_dc_removal
    (
        (float) raw_data.raw_red,
        this->dc_filter_red.w,
        this->dc_alpha
    );

    float mean_diff_res_ir = max30100_mean_diff
    (
        this,
        this->dc_filter_ir.result
    );

    max30100_lpb_filter(this, mean_diff_res_ir);
    ...
    if (max30100_detect_pulse(this, this->lpb_filter_ir.result) && this->samples_recorded > 0) {
        data->pulse_detected = true;
        this->pulses_detected++;

        float ratio_rms = log(sqrt(this->red_ac_sq_sum / (float) this->samples_recorded)) / log(sqrt(this->ir_ac_sq_sum / (float) this->samples_recorded));

        if (this->debug)
            printf("RMS Ratio: %f\n", ratio_rms);

        /**
         * This is my adjusted standard model, so it shows 0.89 as 94%
         * saturation. It is probably far from correct, requires proper
    ...
    ret = max30100_balance_intensities(this, this->dc_filter_red.w, this->dc_filter_ir.w);
    if (ret != ESP_OK)
        return (ret);
...
esp_err_t max30100_balance_intensities
(
    max30100_config_t* this,
    float red_dc,
    float ir_dc
...
{
    if ((uint32_t) (xTaskGetTickCount() * portTICK_PERIOD_MS) - this->last_red_current_check >= this->red_current_adj_ms) {
        if (ir_dc - red_dc > this->acceptable_intense_diff && this->red_current < MAX30100_LED_CURRENT_50MA) {
            this->red_current++;
            esp_err_t ret = max30100_set_led_current(this, this->red_current, this->ir_current);
            if (ret != ESP_OK)
                return (ret);
            if (this->debug)
                printf("RED LED Current +\n");

        } else if (red_dc - ir_dc > this->acceptable_intense_diff && this->red_current > 0) {
            this->red_current--;
            esp_err_t ret = max30100_set_led_current(this, this->red_current, this->ir_current);
            if (ret != ESP_OK)
                return (ret);
        ...
        this->last_red_current_check = (uint32_t) (xTaskGetTickCount() * portTICK_PERIOD_MS);
    }

    return (ESP_OK);
}

esp_err_t max30100_write_register
(
    max30100_config_t* this,
    uint8_t address,
    uint8_t val
...
esp_err_t max30100_read_register
(
    max30100_config_t* this,
    uint8_t address,
    uint8_t* reg
)
{
    ...
    i2c_master_read_byte(cmd, reg, 1);
...
)
{
    ...
    if (size > 1)
        i2c_master_read(cmd, reg, size - 1, 0);

    i2c_master_read_byte(cmd, reg + size - 1, 1);
...
esp_err_t max30100_set_mode(max30100_config_t* this, max30100_mode_t mode)
{
    ...
    esp_err_t ret = max30100_read_register(this, MAX30100_MODE_CONF, &current_mode_reg);
    
    if (ret != ESP_OK)
        return (ret);
    
    return (max30100_write_register(this, MAX30100_MODE_CONF, (current_mode_reg & 0xF8) | mode));
}
...
esp_err_t max30100_set_high_res(max30100_config_t* this, bool enabled)
{
    ...
    if (ret != ESP_OK)
        return (ret);
    if (enabled)
        return (max30100_write_register(this, MAX30100_SPO2_CONF, previous | MAX30100_SPO2_HI_RES_EN));
    else
        return (max30100_write_register(this, MAX30100_SPO2_CONF, previous & ~MAX30100_SPO2_HI_RES_EN));
}
...
esp_err_t max30100_set_sampling_rate(max30100_config_t* this, max30100_sampling_rate_t rate)
{
    ...
    esp_err_t ret = max30100_read_register(this, MAX30100_SPO2_CONF, &current_spO2_reg);
    
    if (ret != ESP_OK)
        return (ret);
    
    return max30100_write_register(this, MAX30100_SPO2_CONF, (current_spO2_reg & 0xE3) | (rate << 2));
}
...
esp_err_t max30100_set_pulse_width(max30100_config_t* this, max30100_pulse_width_t pw)
{
    ...
    return (max30100_write_register(this, MAX30100_SPO2_CONF, (current_spO2_reg & 0xFC) | pw));
}

esp_err_t max30100_set_led_current(max30100_config_t* this, max30100_current_t red_current, max30100_current_t ir_current)
{
    return (max30100_write_register(this, MAX30100_LED_CONF, (red_current << 4) | ir_current));
}
...
esp_err_t max330100_read_temperature(max30100_config_t* this, float* temperature)
{
    ...
    ret = max30100_write_register(this, MAX30100_MODE_CONF, current_mode_reg | MAX30100_MODE_TEMP_EN);
    ...
    float temp_fraction;
    ret = max30100_read_register(this, MAX30100_TEMP_FRACTION, (uint8_t*) &temp_fraction);
    if (ret != ESP_OK)
        return (ret);
...
void max30100_lpb_filter(max30100_config_t* this, float x)
{
    ...
    this->lpb_filter_ir.v[1] = this->lpb_filter_ir.v[2];

    // Fs = 100Hz and Fc = 10Hz:
    this->lpb_filter_ir.v[1] = (2.452372752527856026e-1 * x) + (0.50952544949442879485 * this->lpb_filter_ir.v[0]);
...
esp_err_t max30100_print_registers(max30100_config_t* this)
{
    ...
    if (ret != ESP_OK)
        return (ret);
    ret = max30100_read_register(this, MAX30100_FIFO_OVERFLOW_COUNTER, &fifo_ovf_cnt);
    if (ret != ESP_OK)
        return (ret);
