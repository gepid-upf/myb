 ...
 * the programmer's desired parameters and to update the readings, detecting
 * pulse and having the pulse saturation O2. It is based on Strogonovs Arduino
 * library.
 */
#ifndef MAX30100_H
#define MAX30100_H
...
#define MAX30100_DEFAULT_LED_PULSE_WIDTH            MAX30100_PW_1600US_ADC_16
#define MAX30100_DEFAULT_ACCEPTABLE_INTENSITY_DIFF  65000
#define MAX30100_DEFAULT_RED_LED_CURRENT_ADJUST_MS  500
...
/**
 * Operation Mode enum.
 * Heart rate only or Oxigen saturation plus Heart rate.
 */
...
typedef enum LEDPulseWidth {
    MAX30100_PW_200US_ADC_13                        = 0x00,
    MAX30100_PW_400US_ADC_14                        = 0x01,
    MAX30100_PW_800US_ADC_15                        = 0x02,
    MAX30100_PW_1600US_ADC_16                       = 0x03,
 ...
 * You can also set the debug mode. This functions include some parameters
 * inside that initialize with default values. You can change them later with
 * max30100_set_* functions.
 * 
...
/**
 * @brief Reads from MAX30100 internal registers and updates internal
 * structures. The results are written to a data structure.
 * This functions must be called on a optimal rate of 100Hz.
 * 
 * @param this is the address of the configuration structure.
...
/**
 * @brief Check the difference between RED and IR DC readings. That will enable
 * both LEDs and MAX30100 will start filling the FIFO buffer with readings from
 * both light spectrums.
 *  
 * @param this is the address of the configuration structure.
 * @param red_dc RED DC reading value.
 * @param ir_dc Infrared DC reading value.
 * 
 * @returns status of execution.
 */
 ...
 * This is automatically called by the initializer function.
 * 
 * @param this is the address of the configuration structure.
 ...
 * @returns status of execution.
 */
esp_err_t max30100_set_sampling_rate
(
    max30100_config_t* this, 
 ...
 * This is automatically called by the initializer function or automatically
 * when needed by the library.
 * 
 * @param this is the address of the configuration structure.
 ...
 * @returns status of execution.
 */
esp_err_t max30100_set_led_current
(
    max30100_config_t* this,
 ...
 * This is set to a default (recommended) value in the initializer.
 ...
 * @returns status of execution.
 */
esp_err_t max30100_set_acceptable_intense_diff
(
    max30100_config_t* this,
    uint32_t acceptable_intense_diff
);
 ...
 * @returns status of execution.
 */
esp_err_t max30100_set_red_current_adj_ms
(
    max30100_config_t* this, 
    uint32_t red_current_adj_ms
);
 ...
 * @returns status of execution.
 */
esp_err_t max30100_set_reset_spo2_pulse_n
(
    max30100_config_t* this, 
    uint8_t reset_spo2_pulse_n
);
...
/**
 * @brief Sets the DC filter alpha value.
 * This is set to a default (recommended) value in the initializer.
 * The only way to change is by calling this function.
 * 
 * @param this is the address of the configuration structure.
 ...
 * This is set to a default value in the initializer.
 * The only way to change is by calling this function.
 ...
 * @returns status of execution.
 */
esp_err_t max30100_set_pulse_min_threshold
(
    max30100_config_t* this, 
    uint16_t pulse_min_threshold
);
 ...
 * @returns status of execution.
 */
esp_err_t max30100_set_pulse_max_threshold
(
    max30100_config_t* this, 
    uint16_t pulse_max_threshold
);
...
/**
 * @brief Returns the address of the MAX30100.
 * 
 * @returns address of MAX30100.
 */
